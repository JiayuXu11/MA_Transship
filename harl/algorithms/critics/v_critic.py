...
from harl.utils.models_tools import (
    ...
    update_fragment_schedule_with_decay,
)
...
class VCritic:
    ...
    def fragment_lr_decay(self, episode, episodes):
        """Decay the actor and critic learning rates.
        Args:
            episode: (int) current training episode.
            episodes: (int) total number of training episodes.
        """
        update_fragment_schedule_with_decay(self.critic_optimizer, episode, episodes, self.critic_lr)
    ...
    def share_param_train(self, critic_buffer, value_normalizer=None):
        """Perform a training update using minibatch GD.
        Args:
            critic_buffer: (OnPolicyCriticBufferEP or OnPolicyCriticBufferFP) buffer containing training data related to critic.
            value_normalizer: (ValueNorm) normalize the rewards, denormalize critic outputs.
        ...
        critic_num_mini_batch = self.critic_num_mini_batch * len(critic_buffer)
        critic_epoch = max(self.critic_epoch // len(critic_buffer), 5)
        ...
        for _ in range(critic_epoch):
            data_generators = []
            for critic_buffer_id in range(len(critic_buffer)):
                if self.use_recurrent_policy:
                    data_generator = critic_buffer[critic_buffer_id].recurrent_generator_critic(
                        critic_num_mini_batch, self.data_chunk_length
                    )
                elif self.use_naive_recurrent_policy:
                    data_generator = critic_buffer[critic_buffer_id].naive_recurrent_generator_critic(
                        critic_num_mini_batch
                    )
                else:
                    data_generator = critic_buffer[critic_buffer_id].feed_forward_generator_critic(
                        critic_num_mini_batch
                    )
                data_generators.append(data_generator)
            
            for _ in range(critic_num_mini_batch):
                ...
                value_loss, critic_grad_norm = self.update(
                    tuple(batches), value_normalizer=value_normalizer
                )
                train_info["value_loss"] += value_loss.item()
                train_info["critic_grad_norm"] += critic_grad_norm

        num_updates = critic_epoch * critic_num_mini_batch
