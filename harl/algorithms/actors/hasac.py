...
from harl.models.policy_models.deterministic_policy_mechanism import DeterministicMechanismPolicy
...
class HASAC(OffPolicyBase):
    def __init__(self, args, obs_space, act_space, device=torch.device("cpu")):
        ...
        if act_space.__class__.__name__ == "Box":
            if args.get("mechanism_tf", False):
                self.actor = DeterministicMechanismPolicy(args, obs_space, act_space, device)
            else:
    ...
    def get_actions(self, obs, available_actions=None, stochastic=True):
        ...
        if self.action_type == "Box" and not self.args.get("mechanism_tf", False):
            actions, _ = self.actor(obs, stochastic=stochastic, with_logprob=False)
        elif self.args.get("mechanism_tf", False):
