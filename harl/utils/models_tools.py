...
def update_fragment_schedule_with_decay(optimizer, epoch, total_num_epochs, initial_lr, decay_rate=1/3, decay_epochs_ratio=1/3):
    """Decreases the learning rate with a fragment schedule
    Args:
        optimizer: (torch.optim) optimizer
        epoch: (int) current epoch
        total_num_epochs: (int) total number of epochs
        ...
        decay_epochs_ratio: (float) decay epochs ratio
    """
    decay_times = int(epoch / total_num_epochs / decay_epochs_ratio)
    learning_rate = initial_lr * (decay_rate ** decay_times)
    for param_group in optimizer.param_groups:
        param_group["lr"] = learning_rate
