...
class OffPolicyBufferBase:
    def __init__(self, args, share_obs_space, num_agents, obs_spaces, act_spaces):
        ...
        # 用于判断当前数据可否参与训练
        self.train_available = np.full((self.buffer_size, 1), True)
    ...
    def insert(self, data):
        ...
        Args:
            ...
            train_available: (n_rollout_threads, 1)
        """
        (
            share_obs,
            obs,
        ...
        if self.idx + length <= self.buffer_size:  # no overflow
            ...
            self.train_available[s:e] = train_available.copy()
            ...
            self.train_available[s:e] = train_available[0:len1].copy()
            for agent_id in range(self.num_agents):
            ...
            self.train_available[s:e] = train_available[len1:length].copy()
            for agent_id in range(self.num_agents):
