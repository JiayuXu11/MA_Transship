...
class OffPolicyBufferEP(OffPolicyBufferBase):
    ...
    def sample(self):
        ...
        # 获取train_available为True的索引
        train_available_indices = np.where(self.train_available[:self.cur_size])[0]
        
        # 从可训练的数据中随机采样
        if len(train_available_indices) >= self.batch_size:
            indice = np.random.choice(train_available_indices, size=self.batch_size, replace=False)
        else:
            # 如果可训练的数据不足batch_size，则重复采样
            indice = np.random.choice(train_available_indices, size=self.batch_size, replace=True)
